...
export async function driverFactory () {
  ...
  const { cloneDeep, has, uniq, without } = this.app.lib._
  const { isSet } = this.app.lib.aneka
  ...
  class Driver extends Tools {
    ...
    sanitizeFilter (model, filter) {
      handleRegexInFilter.call(this, filter)
      const { limit, skip, sort, page } = model.preparePagination(filter)
      filter.limit = limit
      filter.skip = skip
      filter.sort = sort
      filter.page = page
    ...
    async _findRecord (model, filter = {}, options = {}) {
      this.sanitizeFilter(model, filter)
