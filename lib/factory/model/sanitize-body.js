 ...
 * @returns {Object}
 */
async function sanitizeBody ({ body = {}, partial, strict, extFields = [], noDefault, truncateString }) {
  ...
  const sanitize = (name, type) => {
    ...
    else if (['string', 'text'].includes(type)) result[name] = sanitizeString(result[name])
    else {
    ...
    if (isNaN(result[name])) result[name] = null
  }
