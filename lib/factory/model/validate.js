...
function buildFromDbSchema (opts = {}) {
 ...
 * @returns {Object}
 */
async function validate (body, joiSchema, opts = {}) {
  const { defaultsDeep } = this.app.lib.aneka
  const { isEmpty } = this.app.lib._
  let { fields, extFields = [], params = {} } = opts

  params = defaultsDeep(params, this.app.dobo.config.validationParams)
  const { rule = {} } = params
  delete params.rule
  if (isEmpty(joiSchema)) joiSchema = buildFromDbSchema.call(this, { fields, rule, extFields })
  if (!joiSchema) return body
  try {
    return await joiSchema.validateAsync(body, params)
  } catch (err) {
    throw this.plugin.error('validationError', { details: err.details, values: err.values, statusCode: 422, code: 'DB_VALIDATION' })
  }
}
