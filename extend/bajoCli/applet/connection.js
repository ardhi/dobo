async function connection (path, ...args) {
  ...
  if (!result) return this.print.fail('cantFind%s%s', this.t('connection'), name, { exit: this.app.bajo.applet })
  this.print.info('done')
  await writeOutput(result, `${this.name}-${path}-${name}`, format, true)
}
